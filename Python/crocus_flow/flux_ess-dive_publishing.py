...
def upload_to_archive(token, base_url, endpoint, json_ld, upload_directory):
    ...
    if post_package_response.status_code == 201:
        ...
        # There was an error
        print("Upload failed!")
        print(f"Status Code: {post_package_response.status_code}")
        print(post_package_response.text)

def main():
    parser = argparse.ArgumentParser(description="Upload data and metadata to the archive.")
    parser.add_argument('--token_file', required=True, help="Path to the text file containing the API token")
    ...
    # Read the token from the specified text file
    ...
    upload_to_archive(token, args.base_url, "packages", json_ld, args.upload_directory)
