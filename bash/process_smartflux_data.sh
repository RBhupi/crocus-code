...
# The script automates the daily processing of SmartFlux data from a waggle-dev-node-w096
# to the CELS GCE server in directory `/nfs/gce/projects/crocus/CMS-FLX-001-UIC`. 
# Each day, the log file is reset, rsync the data and processes to the previous day, 
# checking whether NetCDF files for raw and results data already exist. If not, it calls 
# the processing scripts (read_raw_toNC.py and read_results_toNC.py) and logs errors if processing fails.
# Output NetCDF files are organized in subdirectories named by year and month (%Y%m) for ingest. 
